...
class FormData(MultiDict):
    ...
    def parse_raw(cls, infile):
        return cls.parse(
            infile,
            dict(Header.parse(line) for line in
                 iter(infile.__next__, '\r\n'))['Content-Type'])
    ...
    def handle_upload(self, name, iterable, filename, content_type):
        class FakeFile(str):
            ...
            def __repr__(self):
                return 'Upload(%r, %r, %s)' % (
                    self.filename, self.mimetype, str.__repr__(self))
    ...
    @classmethod
    def parse(cls, infile, content_type_header, content_length, *extra_args,
              **extra_kwargs):
        """
        ...
        def parse_multipart(iterin, post_head, outerboundaries=()):
            ...
            while True:
                headers = dict(Header.parse(line) for line in
                               watch(iterin, '', '\r\n', *outerboundaries))
                if not headers:
                    break
                cd = headers['Content-Disposition']
                name = (cd.get('name') or
                        post_head['Content-Disposition']['name'])
                if 'Content-Type' in headers:
                    ct = headers['Content-Type']
                    if ct.key.startswith('multipart/'):
                        for name, content in parse_multipart(
                                iterin, headers, (sep, term)):
                            yield name, content
                    else:
                        # File upload
                        yield name, self.handle_upload(
                            name, delimited(iterin, '\r\n', '', sep, term,
                                            *outerboundaries),
                            cd.get('filename', ''), ct.key, *extra_args,
                            **extra_kwargs)
                else:
                    # Text value
                    yield name, self.handle_value(
                        name, delimited(
        ...
        if content_type_header.key == 'multipart/form-data':
            self.update(parse_multipart(
                iterin, {'Content-Type': content_type_header}))
