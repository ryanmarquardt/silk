...
Applications are passed two arguments: one Request object and one Response
object
"""

from .. import container, MultiDict, is_sequence
...
class BaseRouter(object):
    ...
    * The handler raises a normal exception. The router calls
      self.report_error, ignoring any exceptions and responds with status code
      500 and the string contained in self.unhandled_error

    """

    unhandled_error = ("The server has encountered a problem and can't"
    ...
    def process(self, request, response):
        try:
            ...
            try:
                ...
                response.content = \
                    response.content or self.unhandled_error or ''
    ...
    def create_request(self, environment):
        ...
        request = container(
            ...
            cookies={m.key: m.value for m in
                     http.cookies.SimpleCookie(
                        environment.get('HTTP_COOKIE', '')
                     ).values()},
            wsgi=container(
            ...
            headers=container(
                content_type=Header.parse_value(
    ...
    def process_input(self, request):
        if request.method == 'POST' and request.headers.content_length:
            ...
            request.post_vars = FormData.parse(
                request.wsgi.input,
    ...
    def _handle(self, environment):
        request, response = \
            self.create_request(environment), self.create_response()
        response.content = self.render(
            self.process(request, response) or response.view,
            response)
        return response
    ...
    def wsgi_handler(self, environment, start_response):
        ...
        start_response(
            format_status(response.code),
            response.headers.as_list())
        return response.content
...
if __name__ == '__main__':
    ...
    router['favicon.ico'] = B64Document.Icon(
        'AAABAAEAEBAAAAEAIABoBAAAFgAAACgAAAAQAAAAIAAAAAEAIAAAAAAAAAQAAAAAAAAA'
        'AAAAAAAAAAAAAAAAAAC/AAAAvgAAALMAAAA+TExMBgAAAABubm5AAAAAcQAAAHAAAABn'
        'AAAAVwAAAC5MTEwU8fHxAQAAAAAAAAAAAAAATgAAAD4AAAArAAAAP0xMTDPx8fEEbm5u'
        'KSsrKz3U1NQMkZGRAQAAAAoAAABbTExMW/Hx8QcAAAAAAAAAAAAAAAAAAAAA8fHxBkxM'
        'TE1MTExN8fHxBgAAAAAAAAAAAAAAAJGRkR0AAABEAAAAT0xMTDvx8fEEAAAAAAAAAAAA'
        'AAAAAAAAAPHx8QlMTExxTExMcfHx8QkAAAAA1NTUDCsrKzgAAABGDg4ORLOzsxUAAAAA'
        'AAAAAAAAAAAAAAAAAAAAAJGRkQQAAAASAAAAf0xMTHzx8fEJAAAAANTU1BYrKytrAAAA'
        'Sg4ODgEAAAAAs7OzEA4ODjORkZEXAAAAAAAAAACRkZELAAAAIgAAAIpMTEyD8fHxCgAA'
        'AADU1NQIKysrKAAAAEkAAABqAAAAWAAAAFgAAABqkZGRLgAAAAAAAAAAkZGREQAAADAA'
        'AACVTExMifHx8QoAAAAA1NTUBSsrKxZubm4P8fHxAUxMTBJMTEwS8fHxAQAAAAAAAAAA'
        'AAAAAJGRkRgAAABAAAAAn0xMTI/x8fELbm5uCQAAACwAAACebm5uavHx8QlMTEx6AAAA'
        'fQAAABaRkZEGAAAAAAAAAACRkZEZAAAAQwAAAJ5MTEyM8fHxC25ubiAAAABEAAAAcAAA'
        'AFwAAAA6AAAAgQAAAIYAAABIkZGRHQAAAAAAAAAAkZGRFgAAADoAAACSTExMg/Hx8Qpu'
        'bm44AAAAXQAAAEYAAABBAAAARwAAAIEAAACBAAAASpGRkR4AAAAAAAAAAJGRkRIAAAAx'
        'AAAAhUxMTHnx8fEJbm5uUAAAAHgAAAAnAAAAEAAAABAAAAAzAAAANAAAABIAAAAmAAAA'
        'OAAAAACRkZEOAAAAKAAAAHlMTExv8fHxCG5ublgrKyuB1NTUGgAAAAAAAAAAAAAAAAAA'
        'AAAAAAAAbm5uKgAAAEoAAAAAkZGRCQAAAB0AAABsAAAAbwAAACYAAABLKysrWtTU1BIA'
        'AAAAAAAAAAAAAAAAAAAAAAAAAG5ubjoAAABmAAAAAAAAAADx8fEGTExMUkxMTFLx8fEG'
        'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA8fHx'
        'BUxMTEYAAABXAAAAOwAAADArKysi1NTUBwAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
        'AAAAAAAAAAAAAPHx8QRMTEw2AAAAeQAAANoAAACUKysrSdTU1A8AAAAAAAAAAAAAAAAA'
        'AAAAAAAAAAAAAAAAAAAAH/8AAP//AAD//wAA//8AAP//AADn/wAA5/8AAOd/AADn5wAA'
        '5+cAAO//AAD+/wAA//8AAP//AAD//wAA+f8AAA=='
    )
    ...
    def upload(request, response):
        ...
        env = r.pop('env')
        return ''.join(map(str, [
            FORM(INPUT(name='upload', type='file'), INPUT(type='submit'),
                 method='post', enctype='multipart/form-data'),
            FORM(INPUT(name='name'), INPUT(type='submit'), method='post',
                 enctype='application/x-www-form-urlencoded'),
            FORM(INPUT(name='name'), INPUT(type='submit'), method='get',
                 enctype='application/x-www-form-urlencoded'),
            PRE(request.vars),
            P(repr(r)), P(repr(env))]))
