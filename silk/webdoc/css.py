...
>>> print(Block(Selector('div'),  # doctest: +NORMALIZE_WHITESPACE
...     Property.new('padding')('0', Units.Px(20), Units.Px(15)),
...     BACKGROUND('transparent'),
...     BORDER(Units.Px(1), 'solid', Colors.Black),
... ))
div { padding: 0 20px 15px; background: transparent;
      border: 1px solid #000000; }

As a shortcut, selectors can be given as strings, and attributes as
dictionaries
...
from .. import container, sequence
...
class Block(CSSNode):
    ...
    def __init__(self, selector, *children, **attributes):
        self.selectors = [Selector(a) if not isinstance(a, CSSNode)
                          else a for a in sequence(selector)]
        mychildren = []
        for child in children:
            if isinstance(child, dict):
                mychildren.extend(Property.new(prop)(*sequence(val))
                                  for prop, val in child.items())
            else:
    ...
    def __str__(self):
        return '%s { %s }' % (
            ', '.join(str(x) for x in self.selectors),
            ' '.join(_css(x) for x in self.children))
...
class Property(CSSNode):
    ...
    background: transparent;
    >>> print(Property.new('padding')(0, Units.Px(0), Units.In(15),
    ...     Units.Px(15)))
    padding: 0 0 15in 15px;

    A single keyword provided as a lower-cased unit name is appended to the
    list of values, transformed into a unitted value. Output with more than
    one keyword input is undefined and not recommended.
    ...
    The special keyword 'important' appends '!important' to the list of
    children if it's set to a true value.
    ...
    def __init__(self, *values, **units):
        ...
        values = list(values)
        if units:
            unit = next(iter(units)).title()
            if unit[0] == '_':
                unit = unit[1:]
            values.extend(Units[unit](x) for x in
                          sequence(units[next(iter(units))]))
        if important:
    ...
    def __str__(self):
        return ' '.join('%s%s: %s;' % (
            p, self.name,
            ' '.join([_css(x) for x in self.children])
        ) for p in [''] + (getattr(self, '__prefixes__') or []))
...
class Selector(CSSNode):
    ...
    def __str__(self):
        ...
        if _child:
            result += '>' + str(_child)
        return ' '.join([result] + [str(x) for x in self.children[1:]])
...
def css(**attributes):
    return [globals()[name.upper().replace('-', '_')](*sequence(value))
...
__all__ = [
    'ANIMATION', 'APPEARANCE', 'BACKFACE_VISIBILITY', 'BACKGROUND', 'BORDER',
    'BORDER_COLLAPSE', 'BORDER_SPACING', 'BOTTOM', 'BOX', 'BOX_SHADOW',
    ...
    'LINE_HEIGHT', 'LIST_STYLE', 'MARGIN', 'MAX_HEIGHT', 'MAX_WIDTH',
    'MIN_HEIGHT', 'MIN_WIDTH', 'NAV', 'OPACITY', 'ORPHANS', 'OUTLINE',
    'OVERFLOW', 'PADDING', 'PAGE_BREAK', 'PERSPECTIVE', 'POSITION',
    'PUNCTUATION_TRIM', 'Property', 'QUOTES', 'RESIZE', 'RIGHT', 'ROTATION',
    'Selector', 'TABLE_LAYOUT', 'TARGET', 'TEXT', 'TOP', 'TRANSFORM',
    'TRANSITION', 'Unit', 'Units', 'VERTICAL_ALIGN', 'VISIBILITY',
    'WHITE_SPACE', 'WIDOWS', 'WIDTH', 'WORD_BREAK', 'WORD_SPACING',
