...
class StencilFile(object):
    ...
    def syntax_error(self, message, code, offset=0):
        raise SyntaxError(
            message, (self.filename, self.lineno, self.column+offset, code))
...
class BaseStencil(object):
    ...
    >>> erb.parse("You have <%= 'no' if x==0 else x %> messages",
    ...           out=sys.stdout)
    ...
    >>> parse("{{if a:}}{{if b:}}{{if c:}}{{=True}}{{pass}}{{pass}}{{pass}}",
    ...       out=sys.stdout)
    if a:
      if b:
        if c:
    ...
    """
    def __init__(self, opener=None, closer=None, indent='\t',
    ...
    def check_syntax(self, stub):
        if stub[-1] == ':':
            if stub[:2] == 'el':
                stub = 'if 1:pass\n' + stub
            elif any(stub.startswith(x) for x in {'except', 'finally'}):
                stub = 'try:pass\n' + stub
            stub += '\n\tpass'
        try:
            compile(stub, self.source.filename, 'exec')
        except SyntaxError as err:
            self.source.syntax_error(err.msg, stub, err.offset)
    ...
    def interpret(self, iprint, python, text, orig):
        ...
        if python:
            ...
            if dedent or any(
                    text.startswith(b) for b in
                    ('elif', 'else', 'except', 'finally')):
                try:
                    iprint(DEDENT)
                except ValueError:
                    self.source.syntax_error(
                        'Got dedent outside of block', orig)
            if not dedent:
    ...
    def compile(self, path_or_file, filename='<string>', out=None):
        self.sources = [StencilFile(
            self.opener, self.closer, path_or_file, filename)]
        _return = out is None
        if _return:
            out = StringIO()
...
class Stencil(BaseStencil):
    ...
    def on_extend(self, printer, token, remainder):
        ...
        self.sources.append(StencilFile(
            self.opener, self.closer, self.open(remainder), remainder))
