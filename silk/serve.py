    ...
    try:
        ...
        if path:
            ...
            if name is None:
                ...
                if base.startswith('..'):
                    raise ImportError(
                        "Must give name and path for modules outside of the"
                        " app's directory")
                name = base.rpartition('.py')[0]
                if '.' in name:
                    raise ImportError(
                        "Cannot import from paths with '.' in them")
                name = name.replace('/', '.')
            ...
            def decorator(handler):  # noqa
                ...
                if handler_nargs == 0:
                    ...
                    if not _Router.route or len(path) > len(_Router.route):
                        _Router.subhandler = \
                            lambda: handler(request.args[len(path):])
                        _Router.route = path
                else:
                    raise TypeError(
                        "Routers must accept zero or one arguments")
                return handler
    return decorator
